...
/* SS_ADD*: newer, faster versions of the above.  Don't mix the two sets
 * of macros.  These are fast because they save reduce accesses to the PL_
 ...
 * like save_pushptrptr() to half its former size.  Of course, doing the
 * size check *after* pushing means we must always ensure there are
 * SS_MAXPUSH free slots on the savestack.  This is ensured by
 * savestack_grow() and savestack_grow_cnt always allocating SS_MAXPUSH
