...
PERL_CALLCONV char*	Perl_pv_escape(pTHX_ SV *dsv, char const * const str, const STRLEN count, const STRLEN max, STRLEN * const escaped, U32 flags);
...
#define PERL_ARGS_ASSERT_UVOFFUNI_TO_UTF8_FLAGS	\
	assert(d)
PERL_CALLCONV U8*	Perl_uvoffuni_to_utf8_flags_msgs(pTHX_ U8 *d, const UV input_uv, UV flags, HV** msgs);
...
#endif
#if defined(PERL_IN_HV_C)
STATIC void	S_clear_placeholders(pTHX_ HV *hv, const U32 placeholders);
...
STATIC const char*	S_stdize_locale(pTHX_ const int category, const char* input_locale, const char **buf, Size_t *buf_size, const line_t caller_line);
...
STATIC void	S_mem_log_common(enum mem_log_type mlt, const UV n, const UV typesize, const char *type_name, const SV *sv, const Malloc_t oldalloc, Malloc_t newalloc, const char *filename, const int linenumber, const char *funcname);
...
#define PERL_ARGS_ASSERT_MEM_LOG_NEW_SV	\
	assert(sv); assert(filename); assert(funcname)
PERL_CALLCONV Malloc_t	Perl_mem_log_realloc(const UV n, const UV typesize, const char *type_name, const Malloc_t oldalloc, Malloc_t newalloc, const char *filename, const int linenumber, const char *funcname);
